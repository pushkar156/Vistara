...
export function CareerRoadmap({ data, userInput, onReset, onViewOpportunities, onBackToRoleSelection }: CareerRoadmapProps) {
  ...
  const handleExportMarkdown = () => {
    const lines: string[] = [
      `# Career Roadmap: ${userInput.desiredCareer}`,
      '',
      userInput.currentRole ? `**Current Background:** ${userInput.currentRole}` : '',
      userInput.interests ? `**Interests:** ${userInput.interests}` : '',
      '',
      '---',
      '',
      ...
      ...(data.roadmap?.beginnerToIntermediate?.map((s, i) => `${i + 1}. ${s}`) || []),
      '',
      '### Intermediate to Pro',
      ...(data.roadmap?.intermediateToPro?.map((s, i) => `${i + 1}. ${s}`) || []),
      '',
      '### Pro to Advanced',
      ...(data.roadmap?.proToAdvanced?.map((s, i) => `${i + 1}. ${s}`) || []),
      '',
      '---',
      '',
      ...
      '',
      '### Beginner to Intermediate',
      ...(data.knowledgeAreas?.beginnerToIntermediate?.map((a) => `- [ ] ${a}`) || []),
      '',
      '### Intermediate to Pro',
      ...(data.knowledgeAreas?.intermediateToPro?.map((a) => `- [ ] ${a}`) || []),
      '',
      '### Pro to Advanced',
      ...(data.knowledgeAreas?.proToAdvanced?.map((a) => `- [ ] ${a}`) || []),
      '',
      '---',
      '',
      '## 3. Recommended Tools',
      ...
      ...(data.advice?.map((a) => `- ${a}`) || []),
      '',
      `*Generated by Vistara - Your Personal Career Counselor on ${new Date().toLocaleDateString()}*`,
    ...
    link.href = url;
    link.download = `${userInput.desiredCareer.toLowerCase().replace(/[^a-z0-9]+/g, '-')}-roadmap.md`;
  ...
  const handleSaveToHistory = async () => {
    ...
    try {
      if (user) {
        ...
        toast({
          title: 'Saved to Cloud History',
          description: `Your roadmap for "${userInput.desiredCareer}" has been saved to your account.`,
        });
      } else {
        // Save locally for guests
        const localKey = 'vistara_guest_history';
        ...
        toast({
          title: 'Saved Locally',
          description: 'Saved on this browser. Sign in anytime to sync your history across all devices.',
        });
      }
    } catch (error: any) {
                ...
                <CardContent className="space-y-4 text-sm">
                  ...
                  {userInput.interests && (
                   ...
                   <Button onClick={onViewOpportunities} variant="secondary" className="w-full mt-4">
                     ...
                     <Button onClick={handleSaveToHistory} disabled={isSaving} className="w-full mt-2">
                        {isSaving ? (
                            <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                        ) : (
                            <History className="mr-2 h-4 w-4" />
                        )}
                        {user ? 'Save to History' : 'Save to Local History'}
                    </Button>
                    <Button onClick={handleExportMarkdown} variant="outline" className="w-full mt-2">
